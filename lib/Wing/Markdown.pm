...
# shamelessly stolen from Text::Markdown and Text::MultiMarkdown
...
#
    ...
    $text =~ s{
        ...
        $self->_GenerateAnchor($whole_match, $link_text, $url, $title);
    }xsge;
...
sub _GenerateAnchor {
    ...
    my ($self, $whole_match, $link_text, $url, $title, $attributes) = @_;
...
#
    ...
    $text =~ s{
        ...
        $self->_GenerateImage($whole_match, $alt_text, $url, $title, $pull);
    }xsge;
...
sub _GenerateImage {
    # FIXME - Fugly, change to named params?
    my ($self, $whole_match, $alt_text, $url, $title, $pull, $attributes) = @_;
    ...
    my $classes = "img-responsive";
    if ($pull) {
        $classes .= " pull-".$pull;
    }

    $result = qq{<img src="$url" alt="$alt_text" class="$classes"};
...
# by default, you get id attributes added to your headers
sub _GenerateHeader {
    my ($self, $level, $id) = @_;
...
sub _DoTables {
    ...
    $text =~ s{
        ...
        my $table = $1;
        my $result = "<table class=\"table table-striped\">\n";
        my @alignments;
        my $use_row_header = 0;
        ...
        if ($table =~ s/^$line_start\[\s*(.*?)\s*\](\[\s*(.*?)\s*\])?[ \t]*$//m) {
            if (defined $3) {
                ...
                $result .= qq{<caption>} . $self->_RunSpanGamut($1). "</caption>\n";
        ...
        while ($alignment_string =~ /\|?\s*(.+?)\s*(\||\Z)/gs) {
            ...
            if ($cell =~ /\:$/) {
                if ($cell =~ /^\:/) {
                    $result .= qq[<col align="center">\n];
                    push(@alignments,"center");
                }
                else {
                    $result .= qq[<col align="right">\n];
                    push(@alignments,"right");
                }
            }
            else {
                if ($cell =~ /^\:/) {
                    $result .= qq[<col align="left">\n];
                    push(@alignments,"left");
                }
                else {
                    if (($cell =~ /^\./) || ($cell =~ /\.$/)) {
                        $result .= qq[<col align="char">\n];
