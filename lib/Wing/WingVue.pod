...
=head1 SYNOPSIS
  ...
  new Vue({
    el : '#app',
    data: {
        users : wing.object_list_manager({
            params : {
                _include_options : 1,
...
Automatically save the value of a form element anytime the model is updated for a C<object_manager>. Saves after 2 seconds, or if the user hits "enter", or if they leave the field.
...
Enables and disables delete confirmations in the  C<object_manager> factory and C<object_list_manager> factory.
...
=head2 wing.object_list_manager()

A factory that generates and manages a list of Wing objects, aka a selection of rows in the database. This is a list of C<wing.object_manager>s.
...
A list of C<object_manager> objects currently managed by the list manager.
...
Updates the object's properties in the database with the object's properties in C<object_manager>, and then returns a promise from the HTTP request. See C<update()> in C<objectManager> for details.
