...
Converts a wing datetime field into a human readable string that you can format.
...
Automatically save the value of a form element anytime the model is updated for a C<new_object_manager>. Saves after 2 seconds, or if the user hits "enter", or if they leave the field.
...
Enables and disables delete confirmations in the  C<new_object_manager> factory and C<new_object_list_manager> factory.
...
=head2 wing.new_object_list_manager()

A factory that generates and manages a list of Wing objects, aka a selection of rows in the database. This is a list of C<wing.new_object_manager>s.
...
A list of C<new_object_manager> objects currently managed by the list manager.
...
Updates the object's properties in the database with the object's properties in C<new_object_manager>, and then returns a promise from the HTTP request. See C<update()> in C<objectManager> for details.
