...
sub extend {
    my $self = shift;
    Wing->log->info(sprintf('SESSION: Extending %s for user %s via process %s', $self->id, $self->user_id, $$));
    ...
    Wing->log->info(sprintf('SESSION: Extended %s for user %s via process %s', $self->id, $self->user_id, $$));
    return $self;
}
...
sub end {
    my $self = shift;
    Wing->log->info(sprintf('SESSION: %s ending via process %s', $self->id, $$));
    Wing->cache->remove($self->key);
    Wing->log->info(sprintf('SESSION: %s ended via process %s', $self->id, $$));
    return $self;
}

sub start {
    my ($self, $user, $options) = @_;
    Wing->log->info(sprintf('SESSION: %s starting for user %s via process %s', $self->id, $user->id, $$));
    ...
    Wing->log->info(sprintf('SESSION: %s started for user %s via process %s', $self->id, $user->id, $$));
    return $self->extend;
}
